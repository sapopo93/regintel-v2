...
export function generateMissingEvidenceFinding(params: {
  ...
  return {
    ...
    title: `Evidence gap: ${params.topic.title} (${params.regSectionPath}) — ${params.missingEvidenceTypes.length} required evidence type(s) missing`,
    description: `The following evidence types required under ${params.regSectionPath} are not uploaded: ${params.missingEvidenceTypes.join(', ')}. Without this evidence, an inspector cannot confirm compliance. This gap would likely contribute to a rating of Requires Improvement or below under the SAF scoring methodology.`,
    severity,
    impactScore,
    likelihoodScore,
    draftedAt,
